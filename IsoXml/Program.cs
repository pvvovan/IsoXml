    ...
    static class Program
    {
        ...
        static void Main(string[] inputArguments)
        {
            ...
            try
            {
                ...
                string isoXmlDirectory = Path.Combine(current, "G_IsoXml");
                    ...
                    foreach (var clDir in clDirs)
                    {
                        ...
                        foreach (var farmDir in farmDirs)
                        {
                            ...
                            foreach (var fieldDir in fieldDirs)
                            {
                                ...
                                {
                                    Shapefile shapeFile = Shapefile.OpenFile(swathesFile);
                        ...
                        foreach (var fa in cl.Farms)
                            foreach (var fi in fa.Fields)
                            {
                                ...
                                foreach (var ou in fi.OuterBoundaries)
                                {
                                    ...
                                    {
                                        f.PLN[ou_num].LSG[0].PNT[p_num] = new XML.field.LSGPNT() { A = "2", C = p.Latitude.ToString("F08"), D = p.Longitude.ToString("F08") };
                                        p_num++;
                                    }
                                    if (p_num > 0)
                                    {
                                    ...
                                    foreach (var io in ou.InnerBoudnaries)
                                    {
                                        ...
                                        {
                                            f.PLN[ou_num].LSG[io_num].PNT[iop_num] = new XML.field.LSGPNT() { A = "2", C = p.Latitude.ToString("F08"), D = p.Longitude.ToString("F08") };
                                            iop_num++;
                                        }
                                        if (iop_num > 0)
                                        {
                                ...
                                foreach (var li in fi.AB_Lines)
                                {
                                    ...
                                    {
                                        f.LSG[li_num].PNT[p_num] = new XML.field.LSGPNT() { A = "2", C = p.Latitude.ToString("F08"), D = p.Longitude.ToString("F08") };
                                        p_num++;
                                    }
                                    if (p_num > 0)
                                    {
                ...
                foreach (var shPolygon in sf.Features)
                {
                    if (shPolygon.ShapeType == ShapeType.Polygon)
                    {
                        ...
                        if (shPolygon.DataRow.ItemArray.Length > 11)
                        {
                            try
                            {
                                xte = double.Parse(shPolygon.DataRow["XTE"].ToString());
                            }
                            catch { }
                            try
                            {
